...
class GtpConnection():
    ...
    def genmove_cmd(self, args):
        ...
        rulebasedMoves = self.board.get_pattern_moves()
        if rulebasedMoves != None:
            if len(rulebasedMoves[1]) > 0:
                move = rulebasedMoves[1]
                move = move[0]
                #print(move)
        else:
            move = MCTS.Monte_Carlo_Tree(self.board.copy(), color)
